...
def get_iterative_upsampling_out(inp, input_depth, w_size, spat_sigma, spec_sigma):
    # Get upsample factor
    ...
    for i in range(uf):
        ...
        D = get_joint_bilateral_out(I, D, w_size, spat_sigma=spat_sigma, spec_sigma=spec_sigma)

    # Resize depth image to match RGB image and apply Joint_Bilateral filter
    D = cv2.resize(D, inp.shape[:2][::-1])
    D = get_joint_bilateral_out(inp, D, w_size, spat_sigma=spat_sigma, spec_sigma=spec_sigma)

    return D


def get_joint_upsampling_out(inp, inp_depth, w_size, spat_sigma, spec_sigma):
    # Get upsample factor
